...
import com.example.ttms.model.EmployeeSale;
import com.example.ttms.model.PlaySale;
...
public interface SaleDAO {
    ...
    @Select("select emp_name,saleAmount from (select emp_id,sum(sale_payment * sale_type) " +
            "as saleAmount from sale where sale_status=1 group by emp_id) emp " +
            "join employee e on emp.emp_id = e.emp_id where e.emp_type = 2;")
    ...
    List<Sale> selectCancelSaleByEmployeeId(Integer id);

    //根据员工name获取销售额
    @Select("select e.emp_name, ss.SaleAmount from (select emp_id,sum(sale_payment*sale_type) " +
            "as SaleAmount from sale where sale_status=1 and sale_type =1 group by emp_id) ss " +
            "join employee e on ss.emp_id = e.emp_id where e.emp_type = 2;")
