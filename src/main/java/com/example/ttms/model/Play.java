...
public class Play {
    ...
    public void byteToStr(){
        if (play_image == null || play_image.length == 0) {
            Base64play_image = "";
        } else {
            Base64play_image = Base64.getEncoder().encodeToString(play_image);
        }
    }

    public void strToByte(){
        if (Base64play_image == null || Base64play_image.trim().isEmpty()) {
            play_image = new byte[0];
        } else {
            play_image = Base64.getDecoder().decode(Base64play_image);
        }
    }
}